...
def test_transfer_plan_transfer_with_no_priorities_forced_to_hold():
    """A 'transfer' verdict with no moves listed is self-contradictory; forced to hold."""
    raw = make_output(
        transfer_priorities=[],
        transfer_plan={
            "recommendation": "transfer",
            "reason": "want to move Rice",
            "expected_gain": 3.0,
            "hit_cost": 4,
        },
    )
    result = parse_adjustments(raw, VALID_IDS)
    assert result.transfer_plan.recommendation == "hold"
    assert result.transfer_plan.expected_gain is None
    assert result.transfer_plan.hit_cost == 0
    assert result.transfer_plan.reason  # some reason was set


def test_transfer_plan_hold_survives_with_empty_priorities():
    """Explicit 'hold' verdict with an empty priorities list is fine as-is."""
    raw = make_output(
        transfer_priorities=[],
        transfer_plan={
            "recommendation": "hold",
            "reason": "rolling for GW6 fixture swing",
            "expected_gain": None,
            "hit_cost": 0,
        },
    )
    result = parse_adjustments(raw, VALID_IDS)
    assert result.transfer_plan.recommendation == "hold"
    assert result.transfer_plan.reason == "rolling for GW6 fixture swing"


def test_transfer_plan_transfer_with_priorities_preserved():
    """A 'transfer' verdict backed by actual moves is left alone."""
    raw = make_output(
        transfer_plan={
            "recommendation": "transfer",
            "reason": "Salah injured",
            "expected_gain": 5.0,
            "hit_cost": 0,
        },
    )
    result = parse_adjustments(raw, VALID_IDS)
    assert result.transfer_plan.recommendation == "transfer"
    assert result.transfer_plan.expected_gain == 5.0
