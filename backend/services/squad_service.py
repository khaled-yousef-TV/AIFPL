...
def assemble_fixed_squad(
    player_predictions: List[Dict],
    player_ids: List[int],
    gameweek: Optional[int],
    method_name: str = "my_team",
    excluded_ids: Optional[List[int]] = None,
) -> Dict[str, Any]:
    """
    Render a known 15-player squad exactly as-is (no MILP rebuild), picking
    only the best legal XI + bench order + captaincy from those 15.

    Used by `my_team` / `tracked` runs so the pitch shows the manager's real
    squad instead of a from-scratch optimizer build. `excluded_ids` still
    excludes from lineup selection (bench them / rotate) but never removes
    a player from the squad — you can't drop players outside a transfer.

    Raises ValueError if fewer than 15 predictions match the given ids, or
    if the 15 don't form a legal squad (2 GK, 5 DEF, 5 MID, 3 FWD, ≤3/team).
    """
    by_id = {p["id"]: p for p in player_predictions}
    squad = [by_id[pid] for pid in player_ids if pid in by_id]
    if len(squad) != len(player_ids):
        missing = [pid for pid in player_ids if pid not in by_id]
        raise ValueError(
            f"Fixed-squad rendering: {len(missing)} of {len(player_ids)} "
            f"players are not in the current prediction pool: {missing}"
        )
    if len(squad) != 15:
        raise ValueError(
            f"Fixed-squad rendering needs exactly 15 players, got {len(squad)}"
        )

    # Legal-shape check: 2 GK, 5 DEF, 5 MID, 3 FWD, max 3 per team.
    from collections import Counter
    pos_counts = Counter(p["position_id"] for p in squad)
    required = {1: 2, 2: 5, 3: 5, 4: 3}
    if any(pos_counts.get(pid, 0) != n for pid, n in required.items()):
        raise ValueError(
            f"Fixed-squad rendering: illegal positional shape {dict(pos_counts)}, "
            "expected 2/5/5/3"
        )
    team_counts = Counter(p["team_id"] for p in squad)
    if any(n > 3 for n in team_counts.values()):
        offenders = {t: n for t, n in team_counts.items() if n > 3}
        raise ValueError(
            f"Fixed-squad rendering: >3 players from a single team: {offenders}"
        )

    # Optimize XI/bench from the fixed 15. Excludes push players to bench.
    excluded = set(excluded_ids or [])
    if excluded:
        # Temporarily zero the predicted points of excluded players so the
        # lineup optimizer benches them naturally — never remove from squad.
        lineup_input = [
            {**p, "predicted": -1.0 if p["id"] in excluded else p["predicted"]}
            for p in squad
        ]
    else:
        lineup_input = squad
    starting_xi, bench, formation = optimize_lineup(lineup_input)
    # Map back to real predicted points for the output (undo the exclusion trick)
    starting_xi = [by_id[p["id"]] for p in starting_xi]
    bench = [by_id[p["id"]] for p in bench]

    if not starting_xi:
        raise ValueError("Could not build a legal XI from the fixed squad.")
    ...
    return {
        ...
        "captain": {"id": captain["id"], "name": captain["name"],
                    "predicted": round(captain["predicted"], 2)},
        "vice_captain": {"id": vice_captain["id"], "name": vice_captain["name"],
                         "predicted": round(vice_captain["predicted"], 2)},
        "total_cost": round(total_cost, 1),
        "predicted_points": round(total_predicted, 1),
        "is_fixed_squad": True,
    }
