...
async def import_fpl_team(team_id: int, gameweek: Optional[int] = None) -> Dict[str, Any]:
    ...
    # Determine gameweek to fetch
    if gameweek is None:
        current_gw = fpl_client.get_current_gameweek()
        next_gw = fpl_client.get_next_gameweek()
        # Try next GW first (most recent), then current GW
        gameweek = next_gw.id if next_gw else (current_gw.id if current_gw else None)
        if not gameweek:
...
def _fetch_team_picks(fpl_client, team_id: int, gameweek: Optional[int] = None) -> tuple:
    """
    Fetch team picks from FPL API with simple fallback logic.
    """
    headers = {
    ...
    if gameweek is None:
        # Try next GW first, then current, then recent past
        if next_gw:
    ...
    # Try gameweeks in order, return first successful
