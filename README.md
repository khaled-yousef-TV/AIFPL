...
The backend start command is `sh scripts/render-start.sh`. It launches `scripts/render-bootstrap.sh` in the background before Uvicorn listens, so the API comes online immediately while first-run data is being fetched. On a new persistent disk the bootstrap downloads current data, builds projections, and creates the first Hermes decision automatically; it skips the work on later deploys once a decision exists. Set `AIFPL_RENDER_BOOTSTRAP=false` if you prefer to initialize manually. The dashboard polls for a few minutes while first-run initialization completes.
